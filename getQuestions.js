...
const text = `Frage №1: In Deutschland dürfen Menschen offen etwas gegen die Regierung sagen, weil …
...
Gebäude mit zwei Türmen rechts und links der Fassade, und einer Glaskuppel auf dem Dach mittig
...
Eine Grafik mit 4 Stimmzetteln: 1) je eine Stimme in der linken und rechten Spalte. 2) zwei Kreuze rechts. 3) zwei Kreuze links 4) drei Kreuze links
✓ 1
— 2
— 3
— 4
...
Willy Brandt kniet auf Stufen vor einem Blumenkranz. Im Hintergrund sind viele Zuschauer zu sehen
...
Schwarz-rot-goldene Flagge mit Hammer, Zirkel und Ährenkranz
— Preußen
...
Eine Grafik mit 4 Flaggen: 1) gestreift, 2) blau mit kreisförmig angeordneten Sternen, 3) heller Hintergrund, Weltkarte, umrandet von Olivenzweigen, 4)
✓ 2
— 1
— 4
— 3
...
const parseQuestionsToJson = (inputText) => {
  const questions = [];
  const questionBlocks = inputText.split(/Frage №\d+:/);
  questionBlocks.shift(); // Remove the first, empty element
  ...
  questionBlocks.forEach((block, index) => {
    ...
    const correctAnswerLetter =
      correctAnswerIndex >= 0
        ? String.fromCharCode('A'.charCodeAt(0) + correctAnswerIndex)
...
const questionsJson = parseQuestionsToJson(text);
const filePath = './questions_output.json';
fs.writeFileSync(filePath, JSON.stringify(questionsJson, null, 2), 'utf8');

console.log(`Questions JSON has been written to ${filePath}`);
